...
### Requirement: Agent executes four domain checks for every request
The agent SHALL invoke all four checks for each request: `check_budget`, `check_vendor_duplication`, `check_policy_compliance`, and `assess_risk`.

#### Scenario: Full check coverage per request
- **WHEN** a purchase request is evaluated
- **THEN** all four domain checks are executed and available for decision synthesis
